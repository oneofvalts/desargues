...
-- def indexer_generator (X Y Z : Projectivization K V) : Fin 3 → Projectivization K V :=
...
(abc_dep : ¬ LinearIndependent K ![a, b, c]) : a ∈ Submodule.span K {b, c} := by
...
have b_span_pq : p ∈ Submodule.span K {p, q} := by {
